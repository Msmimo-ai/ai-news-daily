...
搜索最近24小时AI职场与工业应用新闻，调用 Claude API 处理，生成HTML页面
依赖：anthropic, duckduckgo-search, requests, beautifulsoup4
"""
...
def search_news():
    ...
    with DDGS() as ddgs:
        for query in SEARCH_QUERIES:
            try:
                hits = list(ddgs.news(query, max_results=8, timelimit="d"))  # timelimit=d 表示一天内
                for h in hits:
                    url = h.get("url", "")
...
def process_with_claude(raw_news):
    ...
    print(f"Claude 筛选保留 {len(news_items)} 条新闻")
    return news_items


def generate_html(news_items):
    """调用 generate_page.py 生成 HTML"""
    import subprocess
...
<style>
  body{{font-family:-apple-system,sans-serif;max-width:480px;margin:60px auto;padding:0 20px;color:#1e293b}}
...
{archive_items}
    ...
    with open(index_path, "w", encoding="utf-8") as f:
        f.write(html)
    print(f"index.html 已更新，指向 ai-news-{TODAY}.html")
...
def main():
    ...
    news_items = process_with_claude(raw_news)
